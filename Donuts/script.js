document.addEventListener('DOMContentLoaded', () => {
	...
	peer.on('open', id => {
		document.querySelector("#gameID").value = id;
	...
	document.querySelector("#hostButton").addEventListener("click", e => 
	{
		...
		document.querySelector("#waitingDisplay").classList.remove("hidden");
		
		document.querySelector("#displayGameID").innerHTML = document.querySelector("#gameID").value;
		...
		peer.on('connection', connection => {
				...
				conn.on('open', () => {
					...
					document.querySelector("#waitingDisplay").classList.add("hidden");
					document.querySelector("#game").classList.remove("hidden");
					document.querySelector("#logFieldset").classList.remove("hidden");
			});
		});
	...
	document.querySelector("#joinButton").addEventListener("click", e => 
	{
		...
		conn = peer.connect(document.querySelector("#gameID").value);
	...
	function handleData(data)
	{
		...
		function sendUsername()
		{
			if(conn && conn.open)
			{
				conn.send({username: usernames[isHost ? 0 : 1]});
			}
		}
